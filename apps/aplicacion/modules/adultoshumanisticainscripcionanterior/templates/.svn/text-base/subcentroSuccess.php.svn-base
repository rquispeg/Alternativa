...
<select name="dat_rde_estudiante[subcea_id]" id="subcea_id" style="font-family: Arial;font-size: 12px;width:80px;">
...
<?php echo jquery_ajax(array(
            'listener' => array(
                'selector' => '#subcea_id',
                'event' => 'change'),
            'url' => url_for('adultoshumanisticainscripcionanterior/modalidad'),
            'success' =>  like_function("$('#modalidad').html(data)" , 'data'),
            'data' => "'subcea_id='+". jquery_get('this','val')."+'&ue_id='+document.getElementById('unidad_educativa').value+'&periodo_id='+document.getElementById('periodo_id').value+'&gestion_id='+document.getElementById('gestion_id').value",
            'type' => 'POST'));
?>
