...
<?php echo jquery_ajax(array(
            'listener' => array(
                ...
                'event' => 'change'),
            'url' => url_for('adultoshumanisticainscripcionanterior/nivel'),
            'success' =>  like_function("$('#nivel').html(data)" , 'data'),
            'data' => "'modalidad_id='+". jquery_get('this','val')."+'&ue_id='+document.getElementById('unidad_educativa').value+'&periodo_id='+document.getElementById('periodo_id').value+'&subcea_id='+document.getElementById('subcea_id').value+'&gestion_id='+document.getElementById('gestion_id').value",
            'type' => 'POST'));
?>
