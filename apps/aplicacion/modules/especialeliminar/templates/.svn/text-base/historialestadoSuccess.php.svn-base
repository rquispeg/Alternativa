        ...
        <?php   ui_grid_body_init() ?>
          ...
          <?php foreach ($historialestados as $i => $ins): ?>
            <?php     ui_grid_tr_init() ?>
                ...
                <?php     ui_grid_td_end() ?>
                <?php     ui_grid_td_init('style="text-align:left;"') ?>
                    <?php echo Doctrine::getTable('ClaDiscapacidad')->find($ins->getDiscapacidadId())->getDescDiscapacidad(); ?>
                <?php     ui_grid_td_end() ?>
                <?php     ui_grid_td_init('style="text-align:center;"') ?>
                ...
                <?php     ui_grid_td_init('style="text-align:center;"') ?>
                    <a href="<?php echo url_for('especialeliminar/borrarInscripcion?id_inscripcion='.$ins['id_inscripcion']); ?>" onclick="return confirm('&iquest;DESEA ELIMINAR LA INSCRIPCI&Oacute;N DEL ESTUDIANTE?'); "><img border="0" width="22" height="22" src="/images/deleteins.png" style="border-width: 0px;" title="Eliminar la inscripci&oacute;n del estudiante" /></a>
                <?php     ui_grid_td_end() ?>
