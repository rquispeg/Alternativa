...
<table style="padding: 10px 10px 0px; width: 60%;" align="center">
  ...
  <tbody>
	...
	<tr>
		<td class="caption" colspan="2">Especialidad</td>
	<tr>
	</tr>
	<tr>
		<td colspan="2"><div id="especialidad"><?php include_partial('cea_crear_cursos/comboespecialidades',array('especialidades'=>$especialidades)); ?></div><br></td>
	</tr>
  </tbody>
</table>
...
function enviar()
{
  if(($('#unidad_educativa').val() == -999)||($('#subcea_id').val() == -999)||($('#modalidad_id').val() == -999)||($('#nivel_id').val() == -999)||($('#ciclo_id').val() == -999)||($('#paralelo_id').val() == -999)||($('#turno_id').val() == -999)){
    ...
    window.location = '<?php echo url_for('cea_crear_cursos/guardarcursotecnica'); ?>'+'?subsistema_id='+$('#subsistema_id').val()+'&ue_id='+$('#unidad_educativa').val()+'&subcea_id='+$('#subcea_id').val()+'&periodo_id='+$('#periodo_id').val()+'&modalidad_id='+$('#modalidad_id').val()+'&nivel_id='+$('#nivel_id').val()+'&grado_id='+$('#grado_id').val()+'&paralelo_id='+$('#paralelo_id').val()+'&turno_id='+$('#turno_id').val()+'&curso_oferta_id='+$('#curso_oferta_id').val();
  }
}
/* ]]> */
</script>
