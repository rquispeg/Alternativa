...
DATOS DEL CENTRO DE EDUCACI&Oacute;N ALTERNATIVA
    ...
    <table align="center" style="width:740px;">
        ...
        <tr>
            <td><?php include_partial('adultoshumanisticaextemporanea/combounidadeducativa',array('unidades'=>$unidades)); ?></td>
...
function enviar()
{
  if(($('#unidad_educativa').val() == -999)||($('#subcea_id').val() == -999)||($('#modalidad_id').val() == -999)||($('#nivel_id').val() == -999)||($('#ciclo_id').val() == -999)||($('#paralelo_id').val() == -999)||($('#turno_id').val() == -999)||($('#estado_id').val() == -999)){
    ...
    window.location = '<?php echo url_for('adultoshumanisticaextemporanea/registrarinscripcion'); ?>'+'?ue_id='+$('#unidad_educativa').val()+'&subcea_id='+$('#subcea_id').val()+'&periodo_id='+$('#periodo_id').val()+'&modalidad_id='+$('#modalidad_id').val()+'&nivel_id='+$('#nivel_id').val()+'&grado_id='+$('#grado_id').val()+'&paralelo_id='+$('#paralelo_id').val()+'&turno_id='+$('#turno_id').val()+'&rude='+$('#rude').val()+'&estado_id='+$('#estado_id').val();
  }
}
/* ]]> */
</script>
